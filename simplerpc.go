...
func(service *Service)AddRegistry(registryType regtry.RegistryType, addr string, des string)error{
...
func registerFailed(registryType regtry.RegistryType,addr string){
	switch registryType {
	case 0:
		log.Println(fmt.Sprintf("etcd[%v] register failed!",addr))
	case 1:
		log.Println(fmt.Sprintf("redis[%v] register failed!",addr))
	case 2:
		log.Println(fmt.Sprintf("zookeeper[%v] register failed!",addr))
	case 3:
		log.Println(fmt.Sprintf("consul[%v] register failed!",addr))
	}
}
...
func NewRegistry(registryType regtry.RegistryType, addr string, des string) (regtry.Registry, error) {
...
func(r *Resolver)AddRegistry(registryType regtry.RegistryType, addr string, des string)error{
	registry,err := NewRegistry(registryType, addr, des)
	if err != nil{
		log.Println(err)
		return err
